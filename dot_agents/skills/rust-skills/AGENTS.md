...
name: rust-skills
description: >
  Supplementary Rust reference for topics not covered by rust-guidelines,
  including ownership mechanics, numeric safety, Serde, closures, collections,
  language patterns, and specialized optimization. Use only after
  rust-guidelines when a Rust task specifically needs uncovered guidance.
  Do not use as the default general Rust skill.
license: MIT
metadata:
  author: leonardomso
  version: "1.5.1"
...
Supplementary guide with 256 rules across 26 categories. Use it only to fill a specific gap after consulting `rust-guidelines`; Microsoft guidance is authoritative where the skills overlap. Current for Rust 1.96 (2024 edition).

## When to Apply

Use this skill only when `rust-guidelines` does not cover the needed Rust topic, especially:
- ownership and borrowing mechanics
- numeric and arithmetic safety
- Serde configuration
- pattern matching and closures
- detailed collection selection
- compiler and memory optimization beyond Microsoft's rules

Do not load it as a second general checklist. For overlapping error, API, documentation, testing, workspace, logging, or performance guidance, follow `rust-guidelines` instead.
...
| 20 | Testing | MEDIUM | `test-` | 13 |
| 21 | Documentation | MEDIUM | `doc-` | 12 |
| 22 | Observability | MEDIUM | `obs-` | 7 |
| 23 | Performance Patterns | MEDIUM | `perf-` | 12 |
| 24 | Project Structure | LOW | `proj-` | 12 |
| 25 | Clippy & Linting | LOW | `lint-` | 13 |
| 26 | Anti-patterns | REFERENCE | `anti-` | 14 |
...
This is a fallback reference after `rust-guidelines`, not a parallel default:

1. **Consult `rust-guidelines` first** and stop if it covers the concern.
2. **Select one uncovered category** relevant to the task.
3. **Read only the needed rule file** in `rules/`.
4. **Prefer `rust-guidelines`** if any remaining rule overlaps or disagrees.
