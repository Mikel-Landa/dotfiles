...
Panics indicate the program has reached an unrecoverable state (compare [M-PANIC-IS-STOP](m-panic-is-stop.md) and [M-PANIC-ON-BUG](m-panic-on-bug.md)). Library code in particular should not attempt to catch a panic and continue execution, as there is a risk of observing otherwise impossible state:
