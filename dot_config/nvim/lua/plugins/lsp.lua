...

	{
		...
		config = function(_, opts)
			...
			vim.keymap.set("n", "<leader>ud", function()
				...
				vim.notify(
					diag_enabled and "Diagnostics on" or "Diagnostics off",
					vim.log.levels.INFO
				)
			...
			vim.api.nvim_create_autocmd("LspAttach", {
				...
				callback = function(event)
					...
					map("<C-.>", vim.lsp.buf.code_action, "Code action", { "n", "i", "v" })
					...
					if
						client
						and servers[client.name]
						and type(servers[client.name].keys) == "function"
					then
